...
(function() {
    ...
    let config = {
        name: script.getAttribute('data-name') || 'Chatbot',
        color: script.getAttribute('data-color') || '#3b82f6',
        pos: script.getAttribute('data-pos') || 'right',
        welcome_message: '¡Hola! ¿En qué puedo ayudarte?',
        prompt: script.getAttribute('data-prompt') || ''
    };

    async function init() {
        if (chatbotId) {
            try {
                const response = await fetch(`${domain}/api/bot/config/${chatbotId}`);
                const data = await response.json();
                if (data.success) {
                    config.name = data.name || config.name;
                    config.color = data.color || config.color;
                    config.pos = data.position || config.pos;
    ...
    function render() {
        ...
        chatWindow.innerHTML = `
            <div style="background-color: ${config.color}; padding: 20px; color: white; font-weight: bold; display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="padding: 15px; border-top: 1px solid #eee; display: flex; gap: 10px; background: white;">
                ...
                <button id="chat-send" style="background-color: ${config.color}; color: white; border: none; padding: 0 20px; border-radius: 12px; cursor: pointer; font-weight: bold; transition: opacity 0.2s;">➜</button>
            </div>
        `;
        ...
        function addMessage(text, isUser = false) {
            ...
            if (isUser) {
                msgDiv.style.backgroundColor = config.color;
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (chatbotId) body.chatbot_id = chatbotId;
